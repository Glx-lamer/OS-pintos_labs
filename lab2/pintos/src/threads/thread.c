...
//! Start of lab1 part (5 task)

  add_audit_list(t, THREAD_STARTED);

//! End of lab1 part (5 task)

  /* Add to run queue. */
  thread_unblock (t);

//! Start of lab2 part (schedule call)

  thread_yield();

//! End of lab2 part (schedule call)

  return tid;
}

tid_t
thread_create_burst (const char *name, int priority, int64_t burst,
               thread_func *function, void *aux) 
{
  struct thread *t;
  struct kernel_thread_frame *kf;
...
//! Start of lab1 part (5 task)
  ...
  add_audit_list(thread_current(), THREAD_ENDED);
...
//! Start of lab2 part (add primal_priority)
...
//! End of lab2 part (add primal_priority)
...
//! Start of lab2 part (add new struct elements)

  t->primal_priority = priority;
  list_init(&t->donators);
  t->lock = NULL;

//! End of lab2 part (add new struct elements)
...
//! Start of lab2 part (sorting ready list)

  list_sort(&ready_list, sort_func, NULL);

//! End of lab2 part (sorting ready list)
...
/* Adding element in back of list */
void 
